...
Important scope
---------------
This script evaluates classifier-level group generalization and prevents
paired-decoy leakage. It does NOT by itself retrain the upstream 74
entity-specific QSAR models excluding each held-out EDC. That separate
end-to-end cross-fitting analysis is needed to address potential upstream
annotation-model leakage.

Expected input layout
---------------------
Current working directory/
    leave_one_edc_decoy_group_out_edkg.py
    edkgdl_all_data/             # change GRAPH_DATA_DIR if your folder differs
        Graph_label.txt
        0/Graph_index.txt
        0/Graph_edge_index_direct.txt
        ...
        890/Graph_index.txt
        890/Graph_edge_index_direct.txt

Primary-analysis output (optional, preferred)
----------------------------------------------
The script first tries to load fixed hyperparameters from:
...
If neither is found, it uses MANUAL_HYPERPARAMETERS below.

Expected decoy ordering
-----------------------
Default:
- graph IDs 0–80: 81 EDCs
- graph IDs 81–90: decoys for EDC 0
- graph IDs 91–100: decoys for EDC 1
- ...
- graph IDs 881–890: decoys for EDC 80

If this order is not correct, prepare a mapping CSV with:
...
# Change to "GNN_model_data" if this is your actual current folder name.
GRAPH_DATA_DIR = "edkgdl_all_data"

# The PyG cache will be written to ./processed/
PROCESSED_ROOT = "."
...
# The selected file must contain all 903 zero-based graph IDs.
LABEL_FILE = None
LABEL_FILE_CANDIDATES = ["Graph_label.txt"]

# If decoys are not arranged in sequential blocks of 10 after graph ID 80,
# point this to a CSV with columns: graph_id,parent_edc_id
GROUP_MAPPING_CSV = None

# Change to True only after modifying graph files and needing to rebuild cache.
...
# Internal 891-compound cohort used in this LOGO validation.
...
# Independent 12-compound external cohort, explicitly excluded here.
...
# ----- Pre-specified model configuration -----
# Preferred primary-analysis output from:
# edkgdl_80_20_10fold_offline.py
...
# Alternative primary repeated-nested-CV output:
...
# Used only when no prior primary-analysis output can be found.
# Replace with your finalized primary-model configuration if required.
...
# Must match the primary analysis. The 80/20 offline script used False by
# default to reproduce your original model's CrossEntropyLoss behavior.
MANUAL_USE_CLASS_WEIGHTED_LOSS = False

# ----- Training / prediction -----
# Use fixed epochs, without early stopping, so no held-out group is inspected
# during optimization.
MAX_EPOCHS = 200
...
# Fixed decision rule. 0.5 matches argmax of a two-class softmax model.
...
def append_csv(path: Path, df: pd.DataFrame) -> None:
    """Append rows to a CSV with a header only when creating the file."""
    if df.empty:
        return
    path.parent.mkdir(parents=True, exist_ok=True)
    df.to_csv(
...
class EDKGGraphDataset(InMemoryDataset):
    """Read graph data in the original EDKG-DL directory layout."""
...
def load_labels(label_path: Path) -> np.ndarray:
    """Read graph labels and verify sequential graph IDs."""
...
def build_parent_groups(labels: np.ndarray) -> np.ndarray:
    """
    Return parent-group IDs for the full 903-graph container.

    Internal LOGO groups:
      group i = EDC graph i + 10 paired internal decoys
      decoy block for group i = 87 + 10*i through 96 + 10*i

    External IDs 81–86 and 897–902 are assigned -1, ensuring they cannot enter
    any internal LOGO train/test group.
    """
    n_graphs = len(labels)
...
# =============================================================================
# 4. EDGE-AWARE EDKG-DL MODEL
# =============================================================================

class GCNConvEdge(MessagePassing):
    """Original-style edge-aware directed GCN layer."""
    ...
    def forward(
        ...
        # Edge embeddings participate in the next layer, but are not
        # independently iteratively updated in the node-message equation.
        return out, edge_emb
...
class EDKGDLClassifier(nn.Module):
    """Three edge-aware graph layers, global mean pooling, and FC classifier."""
...
def resolve_fixed_settings() -> Tuple[HyperParameters, bool, str]:
    """
    Resolve pre-specified model configuration from the primary analysis.

    Priority:
    1. selected_model.json from the offline 80/20 + inner 10-fold analysis.
    2. most frequently selected configuration from repeated nested CV.
    3. MANUAL_HYPERPARAMETERS fallback.
    """
...
# =============================================================================
# 6. TRAINING, PREDICTION, AND METRICS
# =============================================================================
...
def create_criterion(
    ...
    """Return loss consistent with the primary analysis."""
...
def predict_probabilities(
    ...
    """Return graph IDs, labels, and EDC probabilities."""
    model.eval()
...
def calculate_metrics(
    ...
    """Calculate ranking and threshold-dependent performance metrics."""
...
def run_one_logo_fold(
    ...
    """
    Train on 80 parent groups and predict one completely withheld parent group.

    No held-out group sample is used for:
    - model fitting,
    - epoch selection,
    - hyperparameter selection,
    - threshold selection.
    """
    # Critical: LOGO validation is performed only on the internal 891 IDs.
    ...
    # Use the same initialization seed for each held-out group. This isolates
    # performance differences caused by the held-out data group rather than
    # arbitrary random initialization differences.
    ...
    # Descriptive group-wise summary. AUC on one-positive groups is stored only
    # for audit; it is not used as the primary evidence.
    ...
    # Rank = 1 means the held-out EDC probability is the highest within its
    # own 11-compound EDC-decoy group.
...
def group_bootstrap_metrics(
    ...
    """
    Compute group-level bootstrap confidence intervals.

    Parent groups, rather than individual chemicals, are resampled with
    replacement. Each bootstrap sample preserves the 1 EDC : 10 decoy
    clustering within every sampled group.
    """
    required = {
    ...
    # Cache each group as arrays to avoid repeated dataframe filtering.
...
def create_pooled_summary(
    ...
    """Compute pooled OOF metrics and fill bootstrap point estimates later."""
    ...
    # Per-group EDC summary supports transparent reporting of which held-out
    # EDC groups were correctly identified at the fixed threshold.
    edc_summary_rows = []
...
# =============================================================================
# 8. RESUME AND CONFIGURATION VALIDATION
# =============================================================================
...
def check_or_write_run_configuration(
    ...
    """
    Save run configuration. For resume mode, prevent accidental mixing of
    results generated with incompatible fixed model settings.
    """
...
def load_completed_groups(output_dir: Path) -> set:
    """Return groups with complete metric and 11 prediction rows."""
...
def main() -> None:
    ...
    # Save all 903 graph IDs and make cohort exclusion auditable.
    ...
    # -------------------------------------------------------------------------
    # Final pooled OOF results and group-level bootstrap CI
    # -------------------------------------------------------------------------
