...
For each model, the script performs grid-search cross-validation, saves the
best fitted model and complete CV results, evaluates the original training and
test sets, and creates a parity plot.

Usage
-----
1. Place this script and the preprocessed CSV files in the same folder.
2. Modify DATA_PREFIX below without adding ".csv".
3. Run:

    python edkgdl_element_regressor.py
...
The following resampled files are read only when present and are not used for
...
# Use a non-interactive backend so figures can be saved in VS Code, terminals,
# servers, and other environments without a display.
matplotlib.use("Agg")
