...
Important note
--------------
This is a ratio sensitivity analysis, not a second primary hyperparameter
search. The main repeated nested-CV analysis should remain the primary
robustness evaluation. Here, fixed hyperparameters are deliberately used to
isolate the effect of changing the number of decoys per EDC.

Expected directory structure
----------------------------
Current working directory/
    negative_ratio_sensitivity_edkg.py
    GNN_model_data/
        Graph_label.txt
        0/Graph_index.txt
        0/Graph_edge_index_direct.txt
        ...
        890/...

Optional primary-analysis result file:
    EDKG_repeated_group_nested_cv_results/
        selected_hyperparameters_per_outer_fold.csv

If the decoys are not ordered in blocks of ten after graph ID 80, provide a
CSV mapping file with columns:
    graph_id,parent_edc_id
and set GROUP_MAPPING_CSV below.
"""
...
# If None, the script first uses Graph_label(1).txt, then Graph_label.txt.
# The selected file must contain the full 903 zero-based graph IDs.
LABEL_FILE = None
LABEL_FILE_CANDIDATES = ["Graph_label.txt"]

# Optional map if DUD-E decoys are NOT in blocks of 10 after graph ID 80.
# Mapping file must contain: graph_id,parent_edc_id
GROUP_MAPPING_CSV = None

# Rebuild cached PyG data only if source graph files have changed.
REPROCESS_DATASET = False

# ----- Zero-based internal/external cohort definition -----
N_EDCS = 81
DECOYS_PER_EDC = 10

# Internal cohort used in this ratio sensitivity analysis.
...
# Independent external cohort excluded from this analysis.
...
# Ratios interpreted as: 1 EDC : K selected decoys
NEGATIVE_RATIOS = [1, 2, 5, 10]

# Number of independently re-sampled decoy sets per ratio.
# 50 is intentionally divisible by 10, 5, 2, and 1 so that decoy exposure
# can be balanced exactly for ratios 1, 2, 5, and 10.
...
# The script will first try to obtain a fixed configuration from the primary
# nested-CV results. It uses the most frequently selected configuration and,
# in a tie, the configuration with the highest mean inner-CV PR-AUC.
AUTO_LOAD_PRIMARY_HYPERPARAMETERS = True
# Default output from the updated first validation script.
# Change this only if you used a different OUTPUT_DIR in that script.
PRIMARY_HPARAMETER_FILE = (
    "EDKG_repeated_group_nested_cv_results_v3/"
    "selected_hyperparameters_per_outer_fold.csv"
)

# Used only if the primary result file is unavailable or AUTO_LOAD... = False.
# Replace these fallback values with your original W&B-selected configuration
# if you have one.
...
# If True, the script skips ratio-round-fold combinations already written to
# outer_fold_metrics.csv and outer_fold_predictions.csv.
...
# =============================================================================
# 2. REPRODUCIBILITY AND INPUT DATA
# =============================================================================
...
class EDKGGraphDataset(InMemoryDataset):
    """Read the existing EDKG graph files in the user's original file layout."""
...
def resolve_label_file(graph_data_dir: Path) -> Path:
    """Prefer the full 903-row label file after the external cohort was added."""
...
def build_parent_groups(labels: np.ndarray) -> np.ndarray:
    """
    Return parent group IDs for the complete 903-graph dataset.

    Internal validation groups:
      group 0: EDC graph 0 + decoys 87–96
      group 1: EDC graph 1 + decoys 97–106
      ...
      group 80: EDC graph 80 + decoys 887–896

    External graph IDs 81–86 and 897–902 are assigned group -1 and are never
    included in decoy sampling or cross-validation.
    """
    n_graphs = len(labels)
...
class GCNConvEdge(MessagePassing):
    """Original-style directed edge-aware graph convolution layer."""
    ...
    def forward(
        ...
        # Every newly introduced self-loop carries a zero edge embedding.
...
class EDKGDLClassifier(nn.Module):
    """Three-layer edge-aware EDKG-DL classifier."""
...
# =============================================================================
# 4. HYPERPARAMETER SELECTION FROM PRIMARY ANALYSIS
# =============================================================================
...
def resolve_fixed_hyperparameters() -> Tuple[HyperParameters, str]:
    """
    Obtain a pre-specified model configuration.

    Priority:
    1. Read primary nested-CV output and select the configuration most often
       selected by inner CV. Ties are broken by mean inner best PR-AUC.
    2. Fall back to MANUAL_HYPERPARAMETERS.
    """
    ...
    if AUTO_LOAD_PRIMARY_HYPERPARAMETERS and primary_path.exists():
        ...
        if required_columns.issubset(hp_df.columns):
            ...
            # If inner_best_pr_auc was absent, mean_inner_pr_auc merely contains
            # counts. It remains a harmless secondary sorting field.
...
# =============================================================================
# 5. GROUP SPLITTING AND BALANCED DECOY SAMPLING
# =============================================================================
...
def group_kfold_splits(
    ...
    """
    Random group-preserving K-fold split.

    Every parent group contains exactly one EDC and the same number of selected
    decoys, so group splitting also preserves class ratios across folds.
    """
...
def group_train_validation_split(
    ...
    """
    Create a parent-group-preserving training/validation split inside one outer
    training set. Validation data remain entirely within outer training data.
    """
...
def make_balanced_decoy_plan(
    ...
    """
    Generate decoy selections with balanced coverage.

    For every EDC parent group, a random permutation of its ten decoys is split
    into non-overlapping chunks of size `ratio`. These chunks are used as
    consecutive sampling rounds. Because 50 is divisible by 10/ratio for the
    requested ratios 1, 2, 5, and 10, every decoy is selected equally often.

    Returns
    -------
    plan[parent_edc_id] -> list of length n_rounds, each item is an array of
    selected decoy graph IDs for that EDC and that sampling round.
    """
...
def indices_for_ratio_round(
    ...
    """Build graph IDs for a complete EDC:decoy-ratio dataset."""
    # All 81 internal EDCs (graph IDs 0–80) remain in every ratio dataset.
...
def save_decoy_selection_audit(
    ...
    """Create a full auditable table of sampled decoy IDs for every ratio/round."""
    audit_rows = []
    ...
    # Verify balanced reuse counts by parent group for transparency.
    coverage = (
...
# =============================================================================
# 6. TRAINING, THRESHOLD SELECTION, AND METRICS
# =============================================================================
...
def get_predictions(
    ...
    """Return graph IDs, labels, and positive-class probabilities."""
    model.eval()
...
def calculate_metrics(
    ...
    """Calculate all requested metrics, with EDC as positive class."""
...
def select_f1_threshold(
    ...
    """
    Choose classification threshold using only validation data.

    Primary criterion: positive-class F1-score for EDCs.
...
def fit_outer_fold_model(
    ...
    """
    Train one model for a sensitivity-analysis outer fold.

    A group-preserving internal validation set is used for:
      1. early stopping using PR-AUC;
      2. positive-class F1 threshold selection.

    Crucially, neither operation accesses the outer test fold.
    """
    ...
    # Select threshold only from the validation output of this best checkpoint.
    _, y_val, p_val = get_predictions(model, validation_loader)
...
# =============================================================================
# 7. METRIC AGGREGATION AND CONFIDENCE INTERVALS
# =============================================================================
...
def repeat_round_metrics_from_folds(
    ...
    """
    Aggregate 5 complementary outer folds within one ratio/sampling round.

    PR-AUC and ROC-AUC are test-size-weighted fold means because each fold
    uses an independently trained model. Threshold-dependent metrics are
    recalculated exactly from summed TP/TN/FP/FN.
    """
...
def bootstrap_ci(
    ...
    """Bootstrap CI for the mean across decoy sampling rounds."""
...
def create_ratio_summary(
    ...
    """Summarize performance distribution separately for each decoy ratio."""
    output_rows = []
...
def load_existing_results(output_dir: Path) -> Tuple[pd.DataFrame, pd.DataFrame]:
    """Load previous partial results when resume mode is enabled."""
...
def already_completed(
    ...
    """Check whether a ratio-round-fold run was completed and saved."""
    if metrics_df.empty:
        return False
...
def append_and_save(
    ...
    """Append one outer fold atomically enough for practical resume support."""
...
def main() -> None:
    ...
    # Kept only for audit clarity. All actual sampled subsets are generated
    # from INTERNAL_EDC_IDS and INTERNAL_NON_EDC_IDS below.
    ...
    # Re-read saved output after all runs, allowing resume mode to be simple.
