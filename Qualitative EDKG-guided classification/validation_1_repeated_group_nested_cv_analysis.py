...
If your decoys are NOT stored in this order, create a CSV file with:
  graph_id,parent_edc_id
and set GROUP_MAPPING_CSV to that file.

Before running
--------------
1. Change GRAPH_DATA_DIR and PROCESSED_ROOT if needed.
2. Confirm that Graph_label.txt and graph folders follow your original layout:
      GNN_model_data/
          Graph_label.txt
          0/Graph_index.txt
...
OUTPUT_DIR = "EDKG_repeated_group_nested_cv_results_v2"

# If None, the script first looks for Graph_label(1).txt, then Graph_label.txt.
# The 903-row label file is required for the zero-based internal/external split.
LABEL_FILE = None
LABEL_FILE_CANDIDATES = ["Graph_label.txt"]

# Optional CSV with columns: graph_id,parent_edc_id.
# Leave as None only if decoys are ordered in 10-decoy blocks after the 81 EDCs.
GROUP_MAPPING_CSV = None

# Rebuild cached PyG data if node/edge files were changed.
...
# Internal cohort used in this repeated nested-CV analysis.
...
# Independent external cohort: deliberately excluded from all nested-CV steps.
...
# Publication setting: 6–10 trials is a reasonable compromise between
# compute cost and hyperparameter exploration. Increase if GPU time allows.
HYPERPARAM_TRIALS = 6
...
# Weighted loss helps the minority EDC class without synthesizing graph samples.
...
# =============================================================================
# 2. REPRODUCIBILITY AND DATASET
# =============================================================================

def seed_everything(seed: int) -> None:
    """Set deterministic seeds as far as practical."""
    random.seed(seed)
    np.random.seed(seed)
...
class EDKGGraphDataset(InMemoryDataset):
    """Load graph files in the same layout used by the original GNN script."""
    ...
    def raw_file_names(self) -> List[str]:
        # PyG only needs a non-empty placeholder list because process() uses
        # absolute paths defined above.
    ...
    def process(self) -> None:
        ...
        for graph_id in range(self.n_graphs):
            ...
            # Assumes column 0 = graph ID and column 1 = label, as in Graph_label.txt.
...
def resolve_label_file(graph_data_dir: Path) -> Path:
    """Prefer the 903-row label file after the external cohort was added."""
...
def build_parent_groups(labels: np.ndarray) -> np.ndarray:
    """
    Build parent-EDC group IDs for the 903-graph dataset.

    Internal validation groups:
      group 0: EDC graph 0 + decoy graphs 87–96
      group 1: EDC graph 1 + decoy graphs 97–106
      ...
      group 80: EDC graph 80 + decoy graphs 887–896

    External graphs 81–86 and 897–902 are assigned group ID -1 and are never
    included in repeated nested-CV splitting, fitting, or scoring.
    """
    n = len(labels)
    ...
    if GROUP_MAPPING_CSV is not None:
        ...
        # Internal EDCs 0–80 each define one parent group.
        groups[INTERNAL_EDC_IDS] = np.arange(N_EDCS)

        # Internal decoys 87–896 occur in 81 consecutive blocks of 10.
        groups[INTERNAL_NON_EDC_IDS] = np.repeat(
            np.arange(N_EDCS),
            DECOYS_PER_EDC,
        )

    # External graphs must remain excluded.
    if np.any(groups[EXTERNAL_GRAPH_IDS] != -1):
        raise RuntimeError(
            "External graph IDs 81–86 and 897–902 must have parent group -1."
        )

    # Validate exactly one internal EDC and 10 internal decoys per parent group.
...
class GCNConvEdge(MessagePassing):
    """
    Edge-aware GCN layer consistent with the original EDKG-DL implementation.

    Node embeddings are updated through graph aggregation. Edge embeddings are
    linearly projected at each layer and injected into the message passed along
    each directed edge.
    """
    ...
    def forward(
        ...
        # Add self-loops. Self-loops receive zero edge embeddings.
...
class EDKGDLClassifier(nn.Module):
    """Three-layer edge-aware graph encoder plus FCNN readout."""
...
def group_kfold_splits(
    ...
    """
    Group-preserving K-fold splitter.

    Because every parent group has exactly one EDC and ten decoys, randomly
    distributing groups across folds preserves the same 1:10 class composition
    in each fold (up to the unavoidable 16/17-group size difference).
    """
...
def class_weights_from_indices(labels: np.ndarray, train_idx: np.ndarray) -> torch.Tensor:
    ...
    # Relative weights: positive class gets approximately n_neg/n_pos times
    # the contribution of the negative class.
...
def choose_threshold(
    ...
    """
    Choose threshold only from inner-CV predictions.

    Primary threshold objective: positive-class F1-score (EDC = 1).
    Tie-breakers: balanced accuracy, then proximity to 0.5.
    """
...
def fit_with_early_stopping(
    ...
    """
    Fit one inner-CV model. Validation PR-AUC determines early stopping.
    Both training and validation groups are inside the outer training split.
    """
...
def fit_fixed_epochs(
    ...
    """Final outer-fold model trained only on the complete outer training set."""
    seed_everything(seed)
    node_channels = dataset.num_node_features
    edge_channels = dataset.num_edge_features
...
def nested_tune(
    ...
    """
    Tune hyperparameters and threshold exclusively inside one outer training set.
    """
    ...
    for config_id, hp in enumerate(configs):
        ...
        # PR-AUC is the primary threshold-independent model-selection metric.
        # Positive-class F1-score and balanced accuracy are tie-breakers.
        key = (
...
def bootstrap_mean_ci(
    ...
    """Bootstrap 95% CI for the mean across outer-fold evaluations."""
...
def main() -> None:
    ...
    # Crucial: repeated nested CV runs only on the internal 891-chemical
    # cohort. External IDs 81–86 and 897–902 are excluded here.
    ...
    for repeat_id in range(N_REPEATS):
        ...
        for outer_fold_id, (outer_train_idx, outer_test_idx) in enumerate(outer_splits):
            ...
            # Safety check: labels returned by the ordered test loader must match
            # the labels indexed from the original label array.
